...
object TimeUtil {
	...
	private fun fromMilliSecondsToMinutes(milli: Long): Long =
		TimeUnit.MINUTES.convert(milli, TimeUnit.MILLISECONDS)

	fun fromMinutesToMilliSeconds(min: Long): Long =
		TimeUnit.MILLISECONDS.convert(min, TimeUnit.MINUTES)
	...
	fun getPastDateFromOffset(offset: Int): String {
		...
		return "${SimpleDateFormat("dd").format(Date(c.timeInMillis))} ${
			getMonth(
				SimpleDateFormat("MM").format(
					Date(c.timeInMillis)
				).toInt()
			)
		}"
	...
	fun getDayFromEpochToDateString(dayFromEpoch: Long): String {
		...
		return "${SimpleDateFormat("dd").format(Date(milli))} ${
			getMonth(
				SimpleDateFormat("MM").format(
					Date(milli)
				).toInt()
